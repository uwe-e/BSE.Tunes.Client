...
{
    /// <summary>
    /// Provides a local HTTP proxy service for streaming audio tracks through a dynamically started server. The proxy
    /// forwards requests to the actual API endpoint, handling authentication and supporting range requests for seeking.
    /// </summary>
    /// <remarks>The LocalProxyService is intended for scenarios where audio tracks must be streamed through a
    ...
    /// guaranteed; callers should ensure that methods are not called concurrently from multiple threads.</remarks>
    public class LocalProxyService : IDisposable
    {
        ...
        public Task<string> StartAsync()
        {
            ...
            // Cache HttpClient
            //_cachedHttpClient = _requestService.GetHttpClientAsync().Result;
        ...
        private async Task HandleRequestAsync(HttpListenerContext context)
        {
            ...
            try
            {
                ...
                //using var response = await _cachedHttpClient.SendAsync(request, HttpCompletionOption.ResponseHeadersRead);
                ...
                //int bytesRead;
                //while ((bytesRead = await apiStream.ReadAsync(buffer.AsMemory(0, BufferSize))) > 0)
                //{
