...
{
    /// <summary>
    /// Provides methods for accessing and managing music-related data, including albums, tracks, playlists, and system
    /// information, by communicating with a remote service endpoint.
    /// </summary>
    /// <remarks>The DataService class acts as a central point for retrieving and updating music library data
    /// from a backend service. It supports operations such as searching, paging, and updating history, and provides
    /// URIs for images and album covers. This class is thread-safe for concurrent use of its methods. All network
    /// operations are performed asynchronously. Exceptions may be thrown if the remote service is unavailable or
    /// returns invalid data.</remarks>
    public class DataService : IDataService
    {
        ...
        public async Task<int> GetAvailableTrackCount()
        {
            return await _requestService.GetAsync<int>($"api/tracks/count");
        }
